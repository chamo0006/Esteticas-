...
function MockAdminUI() {
  return (
    <div className="relative w-full max-w-[480px] mx-auto select-none pointer-events-none">
      ...
      <div className="absolute -inset-8 bg-gradient-to-br from-pink-400/25 via-rose-300/20 to-amber-200/10 rounded-[3rem] blur-3xl animate-drift" />
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-zinc-900/15 border border-white/70 bg-white">
        ...
        <div className="bg-stone-900 px-4 py-2.5 flex items-center gap-3">
          ...
          <div className="flex-1 bg-stone-800 rounded-full px-3 py-1 text-stone-400 text-xs font-mono truncate">
        ...
        <div className="flex" style={{ height: '300px' }}>
          {/* Sidebar */}
          <div className="w-[52px] bg-stone-950 flex flex-col items-center pt-4 gap-3 shrink-0">
            <div className="w-8 h-8 rounded-xl bg-pink-600 flex items-center justify-center mb-1">
              <Sparkles className="w-3.5 h-3.5 text-white" />
            </div>
            {[0, 1, 2, 3].map((i) => (
              <div
                key={i}
                className={`w-8 h-8 rounded-xl flex items-center justify-center ${i === 0 ? 'bg-pink-900/50' : ''}`}
              >
                <div className="w-4 h-0.5 bg-stone-700 rounded" />
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 bg-zinc-50 p-4 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              {[
                { label: 'Turnos',   value: '12',   textColor: 'text-pink-600',    bg: 'bg-pink-50'    },
                { label: 'Clientes', value: '8',    textColor: 'text-rose-600',    bg: 'bg-rose-50'    },
                { label: 'Ingresos', value: '$52k', textColor: 'text-amber-600',   bg: 'bg-amber-50'   },
              ].map((s) => (
            ...
            <div className="space-y-1.5">
              {[
                { time: '10:00', name: 'Ana García',    service: 'Manicura', dot: 'bg-pink-400'    },
      ...
      <div className="absolute -right-3 top-20 bg-white rounded-xl shadow-lg border border-rose-100/80 p-3 w-48 animate-float [animation-delay:0.5s]">
      ...
      <div className="absolute -left-3 bottom-8 bg-white rounded-xl shadow-lg border border-amber-100/80 p-3 w-36 animate-float-alt">
        <p className="text-[10px] text-zinc-500 mb-0.5">Este mes</p>
        <p className="text-2xl font-bold text-zinc-900">+34%</p>
        <div className="flex items-center gap-1 mt-0.5">
          <TrendingUp className="w-3 h-3 text-amber-500" />
          <p className="text-[10px] text-amber-600">reservas online</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border animate-hero">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <Link href="/" className="flex items-center gap-2.5 shrink-0">
            <div className="w-8 h-8 rounded-xl bg-pink-600 flex items-center justify-center shadow-sm shadow-pink-200">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Link
              href="/registrar"
              className="px-4 py-2 bg-pink-600 hover:bg-pink-500 text-white text-sm font-medium rounded-xl transition-colors shadow-sm shadow-pink-200"
            >
              Empezar gratis
            </Link>
          </div>
      ...
      <section id="inicio" className="relative pt-16 pb-28 px-4 sm:px-6 overflow-hidden">
        {/* Animated blobs */}
        <div className="absolute top-0 right-0 w-[560px] h-[560px] bg-pink-100/50 rounded-full -translate-y-1/2 translate-x-1/3 blur-3xl pointer-events-none animate-drift" />
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-amber-100/40 rounded-full translate-y-1/2 -translate-x-1/3 blur-3xl pointer-events-none animate-drift [animation-delay:4s]" />

        <div className="relative max-w-6xl mx-auto grid lg:grid-cols-2 gap-14 lg:gap-10 items-center">
          {/* Copy */}
          <div className="text-center lg:text-left">
            {/* Badge */}
            <div className="animate-hero [animation-delay:0ms] inline-flex items-center gap-2 bg-pink-50 text-pink-700 rounded-full px-4 py-1.5 text-xs font-medium mb-7 border border-pink-200">
            ...
            <h1 className="animate-hero [animation-delay:80ms] font-serif text-5xl sm:text-6xl lg:text-[3.75rem] font-light leading-[1.1] text-foreground mb-6">
              ...
              <em className="not-italic font-semibold text-pink-600">siguiente nivel</em>{' '}
            ...
            <div className="animate-hero [animation-delay:280ms] flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
              <Link
                href="/registrar"
                className="inline-flex items-center justify-center gap-2 px-7 py-3.5 bg-pink-600 hover:bg-pink-500 text-white font-medium rounded-xl transition-all shadow-lg shadow-pink-200 hover:shadow-pink-300 hover:-translate-y-0.5 active:translate-y-0"
              >
            ...
            <div className="animate-hero [animation-delay:380ms] mt-8 flex items-center gap-4 justify-center lg:justify-start">
              <div className="flex -space-x-2">
                {['bg-pink-400', 'bg-rose-400', 'bg-amber-400', 'bg-emerald-400'].map((c, i) => (
      ...
      <section id="beneficios" className="py-24 px-4 sm:px-6 bg-white">
        <div className="max-w-6xl mx-auto">
          <AnimateIn className="text-center mb-16">
            <p className="text-pink-600 font-medium text-sm uppercase tracking-widest mb-3">
              Por qué elegirnos
            </p>
            <h2 className="font-serif text-4xl sm:text-5xl font-light text-foreground mb-4">
              Todo lo que tu negocio{' '}
              <em className="italic font-light text-pink-600">necesita</em>
            </h2>
      ...
      <section id="como-funciona" className="py-24 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <AnimateIn className="text-center mb-16">
            ...
            <h2 className="font-serif text-4xl sm:text-5xl font-light text-foreground mb-4">
              Cómo <em className="italic font-light text-pink-600">funciona</em>
            </h2>
            <p className="text-muted-foreground text-lg max-w-xl mx-auto leading-relaxed">
          ...
          <div className="relative grid md:grid-cols-3 gap-10">
            {/* Connector line — rose to amber (champagne accent) */}
            <div className="hidden md:block absolute top-10 left-[calc(16.67%+2rem)] right-[calc(16.67%+2rem)] h-px bg-gradient-to-r from-pink-200 via-amber-300 to-pink-200" />

            {steps.map(({ n, title, desc }, i) => (
              <AnimateIn key={n} delay={i * 140} direction="up" className="relative text-center flex flex-col items-center">
                <div className="relative mb-6">
                  <div className="w-20 h-20 rounded-full bg-pink-600 flex items-center justify-center text-white font-serif font-light text-3xl shadow-lg shadow-pink-200 relative z-10">
                    {n}
                  </div>
                  <div className="absolute inset-0 rounded-full bg-pink-300/20 scale-[1.4] blur-sm" />
                </div>
          ...
          <AnimateIn className="text-center mt-14" delay={200}>
            <Link
              href="/registrar"
              className="inline-flex items-center gap-2 px-8 py-4 bg-pink-600 hover:bg-pink-500 text-white font-medium rounded-xl transition-all shadow-lg shadow-pink-200 hover:-translate-y-0.5"
            >
      ...
      <section id="precios" className="py-24 px-4 sm:px-6 bg-white">
        <div className="max-w-5xl mx-auto">
          <AnimateIn className="text-center mb-16">
            ...
            <h2 className="font-serif text-4xl sm:text-5xl font-light text-foreground mb-4">
              Invertí en el{' '}
              <em className="italic font-light text-pink-600">crecimiento</em>{' '}
              de tu negocio
            </h2>
            <p className="text-muted-foreground text-lg max-w-xl mx-auto leading-relaxed">
              ...
              <AnimateIn key={plan.name} delay={i * 100} direction="scale">
                <div
                  className={`relative rounded-2xl p-7 border transition-all h-full ${
                    plan.popular
                      ? 'bg-stone-900 border-stone-700 shadow-2xl shadow-stone-900/30 md:scale-105'
                      : 'bg-background border-border hover:shadow-md'
                  }`}
                >
                  {plan.popular && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                      <span className="inline-flex items-center gap-1.5 bg-pink-600 text-white text-xs font-medium px-3.5 py-1.5 rounded-full shadow-md shadow-pink-900/30">
                  ...
                  <div className="mb-6">
                    <p className={`text-xs font-medium uppercase tracking-widest mb-1 ${plan.popular ? 'text-stone-400' : 'text-muted-foreground'}`}>
                      {plan.tagline}
                    </p>
                    ...
                    <div className="flex items-baseline gap-1">
                      ...
                      <span className={`text-sm ${plan.popular ? 'text-stone-400' : 'text-muted-foreground'}`}>
                        /mes
                      </span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-3 mb-7">
                    {plan.features.map((feat) => (
                      <li key={feat} className="flex items-center gap-2.5">
                        <Check className={`w-4 h-4 shrink-0 ${plan.popular ? 'text-pink-400' : 'text-pink-600'}`} />
                        <span className={`text-sm ${plan.popular ? 'text-stone-300' : 'text-muted-foreground'}`}>
                          {feat}
                        </span>
                      </li>
                    ))}
                  ...
                  <Link
                    ...
                    className={`block w-full text-center py-3 rounded-xl font-medium text-sm transition-all ${
                      plan.popular
                        ? 'bg-pink-600 hover:bg-pink-500 text-white shadow-md hover:-translate-y-0.5'
      ...
      <section id="contacto" className="py-24 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <AnimateIn className="text-center mb-16">
            ...
            <h2 className="font-serif text-4xl sm:text-5xl font-light text-foreground mb-4">
              <em className="italic font-light text-pink-600">Hablemos</em>
            </h2>
            <p className="text-muted-foreground text-lg max-w-xl mx-auto leading-relaxed">
      ...
      {/* ── FOOTER ─────────────────────────────────────────────────────────── */}
      <footer className="bg-stone-900 text-stone-400 py-8 px-4 sm:px-6">
