...
import { Plus, Edit2, ToggleLeft, ToggleRight, Loader2, X, Check, Trash2, AlertTriangle, Tag, Upload, ImageIcon } from 'lucide-react';
...
export default function ServiciosPage() {
  ...
  const handleImagenFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 3 * 1024 * 1024) {
      setImgUploadError('La imagen no puede superar 3MB');
      return;
    }
    setImgUploadError(null);
    ...
    formData.append('kind', 'servicio');
    const res = await fetch(`/api/admin/${tenantSlug}/upload`, { method: 'POST', body: formData });
    const data = await res.json().catch(() => ({}));
    if (res.ok) setForm(f => ({ ...f, imagen_url: data.url }));
    else setImgUploadError(data.error ?? 'Error al subir la imagen');
                ...
                <tr key={s.id} className={cn('hover:bg-gray-50 transition-colors', !s.activo && 'opacity-40')}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 bg-gray-50 border border-gray-100 flex items-center justify-center">
                        {s.imagen_url ? (
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-[60] md:p-4">
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-xl w-full md:max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Foto</label>
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-xl border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden flex-shrink-0 bg-gray-50">
                    {imgUploading ? (
                      ...
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={form.imagen_url} alt="Foto del servicio" className="w-full h-full object-cover" />
                    ) : (
                      <ImageIcon className="w-6 h-6 text-gray-300" />
                    )}
                  </div>
                  <div className="flex-1 space-y-1.5">
                    <input ref={imgInputRef} type="file" accept="image/*" className="hidden" onChange={handleImagenFile} />
                    <button
                      type="button"
                      onClick={() => imgInputRef.current?.click()}
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 border-2 border-dashed border-violet-300 rounded-xl text-xs font-medium text-violet-600 hover:bg-violet-50 transition-colors"
                    >
                ...
                {imgUploadError && <p className="text-xs text-red-500 mt-1.5">{imgUploadError}</p>}
                <p className="text-xs text-gray-400 mt-1.5">Se muestra en el catálogo cuando el layout es &quot;Tarjetas&quot; o &quot;Grilla&quot;.</p>
              </div>
              <div>
