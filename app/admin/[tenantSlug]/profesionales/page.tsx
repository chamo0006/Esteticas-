...
export default function ProfesionalesPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      <p className="text-sm text-zinc-500 bg-zinc-100 border border-zinc-200 rounded-xl px-4 py-3 mb-6">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm py-16 text-center">
          <p className="text-zinc-400 text-sm">Todavía no cargaste ningún {singular}.</p>
            ...
            <table className="w-full text-sm min-w-[480px]">
              <thead>
                <tr className="border-b border-zinc-100 text-left">
                  ...
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Especialidad</th>
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Rating</th>
              ...
              <tbody className="divide-y divide-zinc-50">
                {items.map((p) => (
                  <tr key={p.id} className={cn('hover:bg-zinc-50 transition-colors', !p.activo && 'opacity-40')}>
                    <td className="px-6 py-4 font-medium text-zinc-900">{p.nombre}</td>
                    <td className="px-4 py-4 text-zinc-600">{p.rol || <span className="text-zinc-300">—</span>}</td>
                    ...
                    <td className="px-4 py-4">
                      <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-zinc-100 text-zinc-400 hover:text-zinc-600 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-[60] md:p-4">
          <div className="bg-white rounded-t-3xl md:rounded-2xl shadow-xl w-full md:max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
              <h2 className="font-semibold text-zinc-900">{editing ? `Editar ${singular}` : `Nuevo ${singular}`}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Especialidad</label>
                <input
                  value={form.rol}
                ...
                />
                <p className="text-xs text-zinc-400 mt-1">Opcional. Se muestra debajo del nombre.</p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-1.5">Rating</label>
                <input
                  ...
                  placeholder="4.9"
                />
                <p className="text-xs text-zinc-400 mt-1">Opcional. Valor de 0 a 5 (ej: 4.8).</p>
              </div>
            </div>
              ...
              <div>
                ...
                {editing && confirmDelete && (
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setConfirmDelete(false)} className="text-xs text-zinc-500 hover:text-zinc-700">No</button>
                  </div>
                )}
              </div>
